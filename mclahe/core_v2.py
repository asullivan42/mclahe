...
def mclahe(x, kernel_size=None, n_bins=128, clip_limit=0.01, adaptive_hist_range=False, use_gpu=True):
    ...
    with tf.compat.v1.variable_scope("clahe") as scope:
        ...
        tf_x_block = tf.transpose(a=tf.reshape(tf_x_padded, new_shape), perm=perm)
        ...
        perm = tuple(2 * i for i in range(dim)) + tuple(2 * i + 1 for i in range(dim))
        tf_x_hist = tf.transpose(a=tf.reshape(tf_x_hist_padded, new_shape), perm=perm)
        ...
        if adaptive_hist_range:
            ...
            tf_x_hist_max = tf.reduce_max(input_tensor=tf_x_hist, axis=np.arange(-dim, 0))
            tf_x_hist_norm = tf.Variable(tf_x_hist_ex_init, dtype=tf.float32)
            tf_get_hist_min = tf.compat.v1.assign(tf_x_hist_min, tf.reduce_min(input_tensor=tf_x_hist, axis=np.arange(-dim, 0)))
            tf_get_hist_norm = tf.compat.v1.assign(tf_x_hist_norm, tf.compat.v1.where(tf.equal(tf_x_hist_min, tf_x_hist_max),
        ...
        # Clip histogram
        tf_n_to_high = tf.reduce_sum(input_tensor=tf.nn.relu(tf_hist - np.prod(kernel_size) * clip_limit), axis=-1, keepdims=True)
        ...
        tf_cdf_norm = tf.compat.v1.where(tf.equal(tf_cdf_min, tf_cdf_max), tf.ones_like(tf_cdf_max), tf_cdf_max - tf_cdf_min)
        ...
        # Rescaling
        tf_res_min, tf_res_max = (tf.reduce_min(input_tensor=tf_res), tf.reduce_max(input_tensor=tf_res))
        ...
        tf_res_transposed = tf.transpose(a=tf_res, perm=new_shape)
